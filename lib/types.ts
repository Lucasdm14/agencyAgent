...
export interface SocialAccount {
  ...
  display_name: string
  // Publishing credentials — stored client-side only, never sent to server logs
  access_token: string          // Meta/LinkedIn/Twitter token
  page_id?:     string          // For FB/IG business accounts
  webhook_url?: string          // Optional Zapier/Make webhook override
  connected_at: string
}

// ─── Campaign (approved strategy ready to publish) ───────────────────────────
...
// ─── Extended Apify data types ────────────────────────────────────────────────

export interface ApifyInstagramAnalysis {
