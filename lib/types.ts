...
// ─── Brand Assets (visual bank) ───────────────────────────────────────────────

export interface BrandImage {
  id:          string
  name:        string
  url:         string        // base64 data URL (uploaded) or drive/external URL
...
export interface BrandAssets {
  drive_folder_url?: string    // Google Drive public folder
  drive_images:      BrandImage[]   // fetched from drive
  uploaded_images:   BrandImage[]   // manually uploaded
  manual_url?:       string    // logo or brand manual PDF/image URL
  manual_base64?:    string    // uploaded manual as base64
  use_ai_matching:   boolean   // toggle: AI picks best image vs manual
}
...
export interface Post {
  ...
  status:                PostStatus
  context_used?: {
    news_count: number; rss_count: number; competitor_ads_count: number; sources: string[]
  }
  created_at: string
}
...
export interface CopyOption {
  index: number; angle: string; copy: string; hashtags: string[]; rationale: string
}

export interface StrategyPostPlan {
  day: number; platform: string
  content_type: 'informativo' | 'producto' | 'comunidad' | 'educativo' | 'tendencia'
  topic: string; hook_suggestion: string; source_reference: string; visual_direction: string
}
