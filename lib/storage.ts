...
import type { SocialAccount, Campaign } from './types'

const SOCIAL_ACCOUNTS_KEY = 'autocm_social_accounts'
export const getSocialAccounts     = (): SocialAccount[]   => get<SocialAccount[]>(SOCIAL_ACCOUNTS_KEY, [])
export const getBrandSocialAccounts = (brand_id: string): SocialAccount[] =>
  getSocialAccounts().filter(a => a.brand_id === brand_id)
export const upsertSocialAccount   = (a: SocialAccount): void => {
  const all = getSocialAccounts()
  set(SOCIAL_ACCOUNTS_KEY, all.find(x => x.id === a.id) ? all.map(x => x.id === a.id ? a : x) : [...all, a])
}
export const deleteSocialAccount   = (id: string): void =>
  set(SOCIAL_ACCOUNTS_KEY, getSocialAccounts().filter(a => a.id !== id))

// ─── Campaigns ────────────────────────────────────────────────────────────────

const CAMPAIGNS_KEY = 'autocm_campaigns'
export const getCampaigns   = (): Campaign[]        => get<Campaign[]>(CAMPAIGNS_KEY, [])
export const upsertCampaign = (c: Campaign): void => {
  const all = getCampaigns()
  set(CAMPAIGNS_KEY, all.find(x => x.id === c.id) ? all.map(x => x.id === c.id ? c : x) : [c, ...all])
}
export const deleteCampaign = (id: string): void =>
  set(CAMPAIGNS_KEY, getCampaigns().filter(c => c.id !== id))
