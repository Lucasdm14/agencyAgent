import type {
  ...
  ContentStrategy, StrategySession, SocialAccount, Campaign,
} from './types'
...
export const getBrandAgentsByRole  = (bid: string, role: Agent['role']) => getAgents().filter(a => a.brand_id === bid && a.role === role)
...
export const clearStrategySession = (): void => { if (typeof window !== 'undefined') sessionStorage.removeItem(SK) }
...
export const deleteSocialAccount = (id: string): void => set(SAK, getSocialAccounts().filter(a => a.id !== id))

// Competitor analyses
const COMP_K = 'autocm_comp_analyses'
export const getCompetitorAnalyses = (): CompetitorAnalysis[] => get<CompetitorAnalysis[]>(COMP_K, [])
export const addCompetitorAnalysis = (a: CompetitorAnalysis): void => set(COMP_K, [a, ...getCompetitorAnalyses()])
