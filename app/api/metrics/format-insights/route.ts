...
export async function POST(req: Request) {
  ...
  if (body.source === 'apify') {
    ...
    const metrics = await fetchInstagramAccount(body.username, body.brand_id, body.period_days ?? 30)
    if (!metrics) {
      return NextResponse.json(
        { error: `No se encontraron datos para @${body.username}` }, { status: 404 }
      )
    }
    return NextResponse.json({ format_insights: metrics.format_insights })
  }
