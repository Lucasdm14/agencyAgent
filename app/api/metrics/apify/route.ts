...
export async function POST(req: Request) {
  ...
  const body = await req.json() as {
    ...
    page_name?:  string
    period_days?: number
    account_metrics?: object   // for ai_analysis, pass already-fetched metrics
  }
  ...
  if (body.type === 'account') {
    ...
    const metrics = await fetchInstagramAccount(
    ...
    if (!metrics) {
      return NextResponse.json(
        { error: `No se encontraron datos para @${body.username}. Verificá que la cuenta sea pública.` },
        { status: 404 }
      )
    }
  ...
  if (body.type === 'ai_analysis') {
    const metrics = body.account_metrics as { profile: { username: string; followersCount: number }; top_posts: { caption: string; likesCount: number; videoViewCount?: number; type: string; er?: number }[]; format_insights: { best_format: string; format_stats: object; top_hashtags: string[] } }
    ...
    const summary = {
      ...
      top_captions:  (metrics.top_posts ?? []).slice(0, 5).map((p: { caption: string; likesCount: number; videoViewCount?: number; type: string; er?: number }) => ({
