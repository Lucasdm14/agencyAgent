...
export async function POST(req: Request) {
  ...
  const platforms = selected_platforms?.length
    ? selected_platforms
  ...
  try {
    const res = await openai.chat.completions.create({
      ...
      messages: [
        ...
        { role: 'user',   content: `Creá el plan de ${num_days} días para ${period_label}. Redes: ${platforms.join(', ')}.` },
      ],
    })
    const plan = parseJSON<{ pillars: string[]; strategy_rationale: string; posts: StrategyPostPlan[] }>(
