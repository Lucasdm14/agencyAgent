...
import {
  ...
  Heart, MessageCircle, Eye, Play, Layers, Image as ImageIcon,
...
import { getBrands, getSelectedBrandId, saveFormatInsights, getFormatInsights } from '@/lib/storage'

// ─── Helpers ──────────────────────────────────────────────────────────────────

const FORMAT_COLORS: Record<string, string> = {
  Video:   'badge-reel',
  Sidecar: 'badge-carousel',
  Image:   'badge-post',
}
    ...
    <div className="bg-[#161616] border border-[#2a2a2a] rounded-xl overflow-hidden hover:border-[#3a3a3a] transition-colors group">
      ...
      <div className="relative aspect-square bg-[#1a1a1a] overflow-hidden">
        {post.displayUrl ? (
          ...
          <img src={post.displayUrl} alt={post.caption?.slice(0, 40)}
        ...
        {rank && (
          <div className="absolute top-2 left-2 w-6 h-6 rounded-full bg-[#5b6ef5] text-white text-xs font-bold flex items-center justify-center">
            {rank}
          </div>
        )}
        <div className={`absolute top-2 right-2 text-2xs px-1.5 py-0.5 rounded font-medium ${FORMAT_COLORS[post.type] ?? 'badge-post'}`}>
          {post.type === 'Video' ? 'REEL' : post.type === 'Sidecar' ? 'CARR' : 'POST'}
        </div>
        {post.er !== undefined && (
          <div className="absolute bottom-2 left-2 bg-black/70 text-white text-2xs px-2 py-0.5 rounded font-mono">
            ER {post.er}%
          </div>
        )}
      </div>
...
function FormatBar({ fi }: { fi: FormatInsights }) {
  const formats = Object.entries(fi.format_stats ?? {})
  ...
  return (
    <div className="bg-[#161616] border border-[#2a2a2a] rounded-xl p-4 space-y-3">
      <div className="flex items-center justify-between">
        ...
        <span className="text-2xs text-[#5b6ef5] bg-[#5b6ef5]/10 border border-[#5b6ef5]/20 px-2 py-0.5 rounded-full">
          Mejor: {fi.best_format.toUpperCase()} ★
        </span>
      </div>
      {formats.map(([fmt, stats]) => {
        if (!stats) return null
        const maxER = formats[0][1]?.avg_er ?? 1
        const pct   = Math.round((stats.avg_er / maxER) * 100)
        return (
          <div key={fmt} className="space-y-1">
            <div className="flex justify-between text-xs">
              <span className="text-[#a1a1a1] capitalize">{fmt === 'Video' ? 'Reel/Video' : fmt === 'Sidecar' ? 'Carrusel' : 'Post'}</span>
              <span className="text-white font-mono">{stats.avg_er}% ER · {fmtNum(stats.avg_likes)} likes</span>
            </div>
            <div className="h-1.5 bg-[#2a2a2a] rounded-full overflow-hidden">
              <div className="h-full bg-[#5b6ef5] rounded-full transition-all" style={{ width: `${pct}%` }} />
            </div>
            <p className="text-2xs text-[#444]">{stats.count} posts analizados</p>
          </div>
        )
      })}
      {fi.best_posting_hours.length > 0 && (
        <div className="flex items-center gap-2 pt-2 border-t border-[#2a2a2a]">
          ...
          <p className="text-xs text-[#666]">
            Mejores horarios: <span className="text-white">{fi.best_posting_hours.map(h => `${h}:00`).join(', ')}</span>
          </p>
        </div>
      )}
    </div>
...
function AIAnalysis({ metrics }: { metrics: InstagramAccountMetrics }) {
  const [data,    setData]    = useState<Record<string, string[]> & { posting_frequency?: string; generated_at?: string } | null>(null)
  ...
  if (!data && !loading) return (
    <div className="text-center py-12">
      ...
      <p className="text-xs text-[#666] mb-5 max-w-sm mx-auto">
        GPT-4o analiza tus datos reales y extrae insights accionables: hooks que funcionan, formatos, temas y recomendaciones.
      </p>
      <button onClick={run} className="btn btn-primary mx-auto">
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          {data.posting_frequency && <span className="text-xs text-[#666]">· Frecuencia: {data.posting_frequency}</span>}
        </div>
        <button onClick={run} className="btn btn-secondary text-xs py-1 px-2.5"><RefreshCw size={11} /> Regenerar</button>
      </div>
        ...
        {sections.map(({ title, key }) => {
          ...
          return (
            <div key={key} className="bg-[#161616] border border-[#2a2a2a] rounded-xl p-4">
              ...
              {items.length > 0
                ? items.map((item, i) => <p key={i} className="text-xs text-[#a1a1a1] py-0.5">• {item}</p>)
                : <p className="text-xs text-[#444] italic">Sin datos suficientes</p>
              }
            </div>
          )
        })}
      </div>
      {data.generated_at && <p className="text-2xs text-[#444]">Generado: {new Date(data.generated_at).toLocaleString('es-AR')}</p>}
    </div>
  )
}
...
function AdsSection({ handle }: { handle: string }) {
  const [data,    setData]    = useState<{ active_ads: { body_text?: string }[]; new_ads: { body_text?: string }[]; main_messages: string[]; cta_patterns: string[] } | null>(null)
  const [loading, setLoading] = useState(false)
  const [error,   setError]   = useState('')
    ...
    <div className="space-y-4">
        ...
        <div className="bg-[#161616] border border-[#2a2a2a] rounded-xl p-4 space-y-2">
          ...
          {data.main_messages.slice(0, 6).map((m, i) => (
            <div key={i} className="flex gap-2 text-xs">
              <span className="text-[#444] font-mono shrink-0">{i + 1}.</span>
              <p className="text-[#a1a1a1] leading-relaxed">"{m}"</p>
            </div>
          ))}
        </div>
      )}
      {data.cta_patterns.length > 0 && (
        <div className="flex flex-wrap gap-2">
          {data.cta_patterns.map((c, i) => (
            <span key={i} className="text-xs bg-[#5b6ef5]/10 text-[#5b6ef5] border border-[#5b6ef5]/20 px-2.5 py-1 rounded-full">{c}</span>
          ))}
        </div>
      )}
    </div>
...
function AccountView({ brand, periodDays }: { brand: Brand; periodDays: number }) {
  ...
  if (loading) return (
    <div className="flex flex-col items-center gap-3 py-20 text-[#666]">
      ...
      <p className="text-xs text-[#444]">El actor de Apify puede tardar hasta 60 segundos</p>
    </div>
  )
  ...
  return (
    <div className="space-y-6 fade-up">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          {profile.profilePicUrl ? (
            // eslint-disable-next-line @next/next/no-img-element
            <img src={profile.profilePicUrl} alt={handle} className="w-12 h-12 rounded-full border-2 border-[#5b6ef5]/30" />
          ) : (
      ...
      <div className="grid grid-cols-6 gap-3">
        ...
        <KPICard label="Avg. Likes"      value={fmtNum(Math.round(metrics.posts.reduce((s,p)=>s+p.likesCount,0)/Math.max(metrics.posts.length,1)))} />
        <KPICard label="Avg. Comentarios" value={fmtNum(Math.round(metrics.posts.reduce((s,p)=>s+p.commentsCount,0)/Math.max(metrics.posts.length,1)))} />
        <KPICard label="Avg. ER"         value={`${Math.round(metrics.posts.reduce((s,p)=>s+(p.er??0),0)/Math.max(metrics.posts.length,1)*100)/100}%`} />
      ...
      {tab === 'historial' && (
        <div className="space-y-2">
          <p className="text-xs font-semibold text-[#666] mb-3">Todos los posts — {metrics.posts.length} en {periodDays} días</p>
          {metrics.posts
            ...
            .map((post, i) => (
              <div key={i} className="flex items-start gap-4 bg-[#161616] border border-[#2a2a2a] rounded-xl p-4 hover:border-[#3a3a3a] transition-colors">
                ...
                <div className="shrink-0 w-14 h-14 rounded-lg overflow-hidden bg-[#1a1a1a]">
                  {post.displayUrl
                    ? <img src={post.displayUrl} alt="" className="w-full h-full object-cover" /> // eslint-disable-line @next/next/no-img-element
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1">
                    <span className={`text-2xs px-1.5 py-0.5 rounded ${FORMAT_COLORS[post.type] ?? 'badge-post'}`}>
    ...
    <div>
      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-xs text-[#666]">Datos en tiempo real desde Apify · Se guardan automáticamente para la Estrategia</p>
        </div>
      ...
      <div className="tab-bar w-fit mb-6">
        <button className={`tab ${tab === 'propia' ? 'active' : ''}`} onClick={() => setTab('propia')}>
          Cuenta propia {brand.instagram_handle && <span className="ml-1 text-2xs text-[#666]">@{brand.instagram_handle.replace('@','')}</span>}
        </button>
        <button className={`tab ${tab === 'competidores' ? 'active' : ''}`} onClick={() => setTab('competidores')}>
          Competidores {brand.competitors?.length > 0 && <span className="ml-1 text-2xs bg-[#2a2a2a] px-1.5 py-0.5 rounded">{brand.competitors.length}</span>}
        </button>
      </div>
          ...
          {!brand.competitors?.length ? (
            ...
            brand.competitors.map((c, i) => (
              <CompetitorRow
                key={i}
                name={c.name}
                handle={c.instagram_handle ?? c.name.toLowerCase().replace(/\s/g, '')}
                brandId={brand.id}
              />
            ))
          )}
