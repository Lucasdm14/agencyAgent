...
import { getBrandAgents, upsertAgent, deleteAgent } from '@/lib/storage'
...
function AgentForm({ agent, brandId, brands, onSave, onCancel }: {
  ...
  // Update base prompt when role changes (only for new agents)
  ...
  return (
    <div className="bg-card border border-border rounded-xl p-6 space-y-5 fade-up">
      ...
      {/* Segment (all roles can have it, most relevant for copy agents) */}
      <div className="space-y-1.5">
...
export default function AgentsPage() {
  ...
  useEffect(() => {
    ...
    setAgents(all)
    if (brands.length > 0) setExpanded(prev => Object.keys(prev).length === 0 ? { [brands[0].id]: true } : prev)
  }, [brands])
